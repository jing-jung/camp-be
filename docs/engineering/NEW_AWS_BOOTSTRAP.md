...
8. GitHub Actions의 `backend-dev-deploy` workflow를 수동 실행한다.
   ...
   `envs/dev-junwoo/deploy.auto.tfvars.json`을 생성한 뒤 plan-only로 검증한다.
   plan 결과를 확인한 뒤 실제 배포가 필요할 때만 같은 `target_env`로
   `apply=true`를 선택해서 다시 실행한다.
...
- 생성된 profile 파일로 Terraform init/plan을 실행한다.
- Lambda package를 만들어 배포 전 package 단계까지 검증한다.

`apply=false`는 수동 실행의 기본값이며 AWS 리소스를 변경하지 않는 검증
모드다. plan을 검토한 뒤 실제 dev 배포가 필요할 때만 같은 화면에서
`apply=true`를 선택한다. `main` branch push로 실행되는 자동 배포는 기존처럼
plan 후 apply까지 진행한다.
...
3. `backend-dev-deploy` workflow를 아래 값으로 실행한다.
   ...
   이 실행은 account, backend, tfvars, OIDC role, Terraform plan을 먼저
   검증한다. plan 결과가 의도와 맞을 때만 같은 `target_env`로 `apply=true`
   수동 실행을 다시 시작한다.
